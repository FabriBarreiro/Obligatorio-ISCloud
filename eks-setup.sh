...
helm upgrade --install monitoring prometheus-community/kube-prometheus-stack \
  ...
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/scheme=internet-facing" \
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/target-type=ip" \
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/listen-ports=[{\"HTTP\":80}]" \
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/healthcheck-path=/api/health" \
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/success-codes=200" \
  --set-string "grafana.ingress.annotations.alb\.ingress\.kubernetes\.io/inbound-cidrs=0.0.0.0/0"
